...
fn test_change_milestone_status_and_approved() {
    ...
    let result =
        escrow_approver.try_approve_milestone(&invalid_index, &approver_address);
    assert!(result.is_err());
    ...
    let result =
        escrow_approver.try_approve_milestone(&(0 as i128), &unauthorized_address);
    assert!(result.is_err());
}
...
fn test_release_funds_milestones_incomplete() {
    ...
    let result = escrow_approver.try_release_funds(&release_signer_address, &wardchain_address);
    assert!(result.is_err());
}
...
fn test_get_multiple_escrow_balances_platform_authorized() {
    ...
    let res_two = c1.get_multiple_escrow_balances(&vec![&env, c1.address.clone(), c2.address.clone()]);
