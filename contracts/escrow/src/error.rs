    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ContractError::MilestoneToApproveDoesNotExist => {
                write!(f, "The milestone to approve does not exist")
            }
            ContractError::EscrowNotFullyProcessed => {
                write!(f, "The escrow must be fully processed before withdrawing remaining funds")
            }
            ContractError::TooManyDistributions => {
                write!(f, "Cannot define more than 50 distributions when resolving dispute")
            }
